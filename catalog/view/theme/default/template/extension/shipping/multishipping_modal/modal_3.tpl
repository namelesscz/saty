                ...
                <div class="row-fluid">
                  ...
                  <div class="col-sm-12">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="modal_3_city"<?php if (!empty($shipping_city)) { ?> value="<?php echo $shipping_city; ?>" <?php } ?> />
                      <input type="hidden" id="modal_3_city_branch_id" />
                    </div>
                  </div>
            ...
            <div class="modal-footer">                        
              ...
              <button type="button" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-success select_destination hidden" data-service_id="3"><?php echo $text_select_destination; ?></button>
            </div>
            
<script type="text/javascript">
$('#search_destinations').submit(function () {
  var modal_3_city = $('input#modal_3_city_branch_id').val();
    
  $('#search_destinations').find('button[type="submit"]').button('loading');  
  
  $.ajax({
    url: 'index.php?route=extension/shipping/multishipping/search_service_3&service_id=3&branch_id=' + encodeURIComponent(modal_3_city),
  	dataType: 'json',
...
$('input#modal_3_city').autocomplete({
  	source: function(request, response) {
  		$.ajax({
  			url: 'index.php?route=extension/shipping/multishipping/autocomplete&service_id=3&filter_what=city&filter_value=' +  encodeURIComponent(request),
  			dataType: 'json',
  			success: function(json) {  
	...
	select: function(item) {
		$('input#modal_3_city').val(item['label']);
		$('input#modal_3_city_branch_id').val(item['value']);
