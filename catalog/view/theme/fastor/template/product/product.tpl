     			           ...
     			           <div id="input-option<?php echo $option['product_option_id']; ?>">
     			             ...
     			             <div class="radio <?php if($theme_options->get( 'product_page_radio_style' ) == 1) { echo 'radio-type-button2'; } ?>">
     			               <label>
     			                 ...
     			                 <span <?php if ($option_value['image'] && 0) { echo 'style="padding: 2px"'; } ?> <?php if ($option_value_id == $option_value['product_option_value_id']){ echo "class='active'"; } ?>><?php if (!$option_value['image'] || 1) { ?><?php echo $option_value['name']; ?><?php } ?>
     			                 <?php if ($option_value['image'] && 0) { ?>
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				glami('track', 'AddToCart', {
					item_ids: ['<?php echo $product_id; ?>'], // product ID currently added to a cart. Use the same ID as you use in the feed (ITEM_ID).
					product_names: ['<?php echo $heading_title; ?>'], // product name currently added to a cart. Use the same names as you use in the feed (PRODUCTNAME).
				});
...
$.fn.tabs = function() {
	...
	glami('track', 'ViewContent', {
		...
		item_ids: ['<?php echo $product_id; ?>'], // currently viewed product ID. Use the same ID as you use in the feed (ITEM_ID)
		product_names: ['<?php echo $heading_title; ?>'] // currently viewed product name. Use the same names as you use in the feed (PRODUCTNAME).
	});
