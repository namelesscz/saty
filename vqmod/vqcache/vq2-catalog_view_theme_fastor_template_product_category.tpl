  			...
  			<div class="name-actions col-sm-4">
  				...
  				<ul>
  				     <?php if($theme_options->get( 'display_add_to_cart' ) != '0') { ?>
  				          ...
  				          <?php } else { ?>
  				          <li><a onclick="addButtonChecker.AddToCart('<?php echo $product['product_id']; ?>', '<?php if (isset($product['minimum'])) echo $product['minimum']; else echo "1"; ?>');cart.add('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" data-original-title="<?php echo $button_cart; ?>"><i class="fa fa-shopping-cart"></i></a></li>
  				          <?php } ?>
  				     <?php } ?>
  					...
  					<li><a onclick="<?php if (isset($product['product_id'])) $wishlist_product_id=$product['product_id']; else $wishlist_product_id=$product_id; ?>addButtonChecker.AddToWishlist('<?php echo $wishlist_product_id; ?>');wishlist.add('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" data-original-title="<?php if($theme_options->get( 'add_to_wishlist_text', $config->get( 'config_language_id' ) ) != '') { echo $theme_options->get( 'add_to_wishlist_text', $config->get( 'config_language_id' ) ); } else { echo 'Add to wishlist'; } ?>"><i class="fa fa-heart"></i></a></li>
  					<?php } ?>
  				</ul>
  			</div>
